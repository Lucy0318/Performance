...
#serch for  Frame Rate
   ...
   def create_MaxRoutes_reportFile(self,path,filename):
   	    csvfile = path+'/'+filename
   	    a = open(csvfile,'wb')
   	    fileheader=["Target","Default","Actual","Comments",]
   	    dict_writer = csv.DictWriter(a,fileheader)
   	    dict_writer.writeheader()
   	    a.close()	
...
#search all row, get Frame date rate value
   ...
   def get_Ixload_ClientResult(self,path,filename):
   	    xlsxfile = path+'/'+filename
   	    data=load_workbook(xlsxfile,'r')
   	    sheetnames = data.sheetnames
   	    table=data[sheetnames[0]]
...
##   	    tmp = tmp.replace(',','')
   ...
   def get_Ixload_ServerResult(self,path,filename):
   	    xlsxfile = path+'/'+filename
   	    data=load_workbook(xlsxfile,'r')
   	    sheetnames = data.sheetnames
   	    table=data[sheetnames[0]]
...
##   	    tmp = tmp.replace(',','')
   ...
   def get_IxnetworkResult(self,path,filename,lenth):
   	    result=[lenth,0,0,0]
   	    xlsxfile = path+'/'+filename
   	    data=load_workbook(xlsxfile,'r')
   	    sheetnames = data.sheetnames
   	    table=data[sheetnames[0]]
   	    agg = table.cell(2,7).value
   	    averge = float(agg)
   	    print('averge Frame Data Rate value is %f'%averge) 
   	    result[1]=averge
#search for Minimum Frame Latency

   	    result[2]=table.cell(2,12).value
#serch for  Frame Rate
            result[3]=table.cell(2,6).value
   	    return result

   def getcurrdir(self,path):
           os.chdir(path)
           tmp = os.getcwd()
           tmp1 = re.sub(r'\\', r'\\\\', tmp)
           return tmp1

   def open_IxloadGW(self):
        try:
           os.popen('NET STOP IxLoadGatewayService')
           time.sleep(60)
        except OSError:
           print ("system error")
        os.popen('NET Start IxLoadGatewayService')    
